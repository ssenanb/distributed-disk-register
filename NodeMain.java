...
public class NodeMain {
    ...
    // -----------------
    private static int TOLERANCE = 1; // tolerance.conf'tan okunacak değer
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (commandType.equals("SET")) {
                    if (parts.length < 3) {
                        ...
                        try {
                            ...
                            //6. aşama için değiştirildi
                            List<NodeInfo> candidates = registry.snapshot().stream()
                                    .filter(n -> n.getPort() != self.getPort()) // Kendini çıkar
                                    .sorted(java.util.Comparator.comparingInt(NodeInfo::getPort)) // Porta göre sırala
                                    .collect(java.util.stream.Collectors.toList());

                            if (!candidates.isEmpty()) {
                                // 3. Round-Robin Başlangıç Noktası Belirle
                                // Sayaç her istekte 1 artar. Modulo (%) işlemi ile başa döner.
                                int currentIndex = requestCounter.getAndIncrement() % candidates.size();

                                int sentCount = 0;
                                // Dairesel döngü: Listeyi "currentIndex"ten başlayarak dönüyoruz
                                for (int i = 0; i < candidates.size(); i++) {
                                    if (sentCount >= TOLERANCE) break; // Yeterince kişiye gönderdiysek dur

                                    // (Başlangıç + i) % Toplam Sayı formülü ile listeyi dairesel geziyoruz
                            ...
                            // 4. Haritayı güncelle ve Cevap Dön
                ...
                // --- GET KOMUTU ---
                    ...
                    if (parts.length < 2) {
                        ...
                        try {
                            int id = Integer.parseInt(parts[1]);
                            String result = storageService.get(id); // 1. Önce liderden oku

                            if (result == null) { // 2. Liderde yoksa başkasından iste
                                List<NodeInfo> locations = messageLocations.get(id);
                                if (locations != null) {
                                    for (NodeInfo loc : locations) {
                                        if (loc.getPort() == self.getPort()) continue;
                                        result = callRetrieveRpcSync(loc, id); // SYNC metodunu çağırıyoruz
    ...
    private static void startFamilyPrinter(NodeRegistry registry, NodeInfo self) {
        ...
        scheduler.scheduleAtFixedRate(() -> {
            ...
            for (NodeInfo n : members) {
                ...
                if (isMe) {
                    currentMsgCount = storageService.getCount(); // Kendi sayımızı lokalden al
                } else {
